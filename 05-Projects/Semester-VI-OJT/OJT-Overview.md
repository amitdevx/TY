---
title: Semester VI OJT (On-Job Training)
status: pending
semester: VI
tags:
  - ojt
  - project
  - semester-vi
aliases:
  - OJT Sem VI
  - On-Job Training
created: 2026-06-16
last_modified: 2026-06-16
---

[[00-Dashboard/Home|Home]] | [[05-Projects/Projects-Dashboard|Projects Dashboard]]


# Semester VI - OJT (On-Job Training) Overview

> [!important] OJT Status
> **Status:**  Pending | **Semester:** VI | **Duration:** ___ weeks

---

## Organization Details

| Field | Details |
|-------|---------|
| **Organization Name** | |
| **Industry/Sector** | |
| **Address** | |
| **Website** | |
| **Contact Person** | |
| **Contact Email/Phone** | |
| **Mentor/Supervisor Name** | |
| **Department** | |
| **OJT Start Date** | |
| **OJT End Date** | |
| **Working Hours** | |
| **Mode** | ⬜ On-site  ⬜ Remote  ⬜ Hybrid |

---

## Learning Objectives

By the end of this OJT, I aim to:

1. **Technical Skills:**
   - [ ] Learn _______________
   - [ ] Understand how _______________
   - [ ] Implement _______________

2. **Soft Skills:**
   - [ ] Improve professional communication
   - [ ] Work in a team environment
   - [ ] Manage time and deadlines in industry setting

3. **Domain Knowledge:**
   - [ ] Understand the industry domain: _______________
   - [ ] Learn about production-level software practices
   - [ ] Understand DevOps / CI-CD pipelines (if applicable)

---

## Project/Work Assigned

### Project Title
> _______________

### Project Description
> _______________

### My Role
> _______________

### Technologies Used
| Technology | Purpose |
|-----------|---------|
| | |
| | |
| | |

---

## Daily Log Template

> [!tip] Fill this every working day
> Copy this template for each day and fill it consistently.

### Daily Log - [DATE]

```
Date       : 
Day Number : Day ___ of ___
Tasks Done :
  1. 
  2. 
  3. 
New Learning :
  - 
Problems Faced :
  - 
Solutions Found :
  - 
Tomorrow's Plan :
  1. 
  2. 
Supervisor Feedback :
  - 
```

---

## OJT Deliverables

### Weekly Deliverables
- [ ] Week 1 Report submitted
- [ ] Week 2 Report submitted
- [ ] Week 3 Report submitted
- [ ] Week 4 Report submitted
- [ ] Week 5 Report submitted
- [ ] Week 6 Report submitted

### Final Deliverables
- [ ] Daily OJT log (complete)
- [ ] OJT Certificate from organization
- [ ] Completion Letter
- [ ] Final OJT Report submitted to college
- [ ] Viva/Presentation (if required)

---

## Final Report Structure

The OJT final report should include:

### Chapter 1: Introduction
- About the organization
- Vision, mission, and products/services
- Organizational hierarchy

### Chapter 2: Project Description
- Project title and objective
- Scope of the project
- Technologies and tools used

### Chapter 3: Work Done
- Week-wise summary of activities
- Technical tasks performed
- Contribution to the organization

### Chapter 4: Learning Outcomes
- Technical skills gained
- Soft skills developed
- Industry exposure

### Chapter 5: Conclusion
- Summary of OJT experience
- Future scope
- Acknowledgements

### Appendices
- Daily logs
- Code snippets (if applicable)
- Screenshots / outputs
- OJT certificate

---

## Related Notes

- [[Progress-Tracker]] - OJT weekly progress
- [[02-Semester-VI/CS-351-MJ-T-Advanced-Java/Overview|Overview]] - Semester VI overview
- [[06-Revision/Final-Revision/Semester-VI-Final]] - Semester VI final revision

---

*Last updated: 2026-06-16 | Semester VI TY B.Sc. CS*
