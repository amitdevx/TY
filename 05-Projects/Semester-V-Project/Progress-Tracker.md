---
title: Project Progress Tracker
tags:
  - project
  - tracking
  - semester-v
aliases:
  - SemV Progress
created: 2026-06-16
last_modified: 2026-06-16
---

#  Semester V Project - Progress Tracker

> [!note] Quick Links
> [[Project-Overview]] | [[06-Revision/Final-Revision/Semester-V-Final]]

---

## ️ Phase-Wise Progress

### Phase 1: Planning
- [ ] Problem statement finalized
- [ ] Objectives defined
- [ ] Tech stack selected
- [ ] Team roles assigned
- [ ] Mentor meeting scheduled
- [ ] Timeline created

**Phase 1 Completion:** 0 / 6 tasks ⬜

---

### Phase 2: Design
- [ ] Use case diagrams created
- [ ] ER diagram finalized
- [ ] System architecture designed
- [ ] UI wireframes ready
- [ ] Database schema approved

**Phase 2 Completion:** 0 / 5 tasks ⬜

---

### Phase 3: Development
- [ ] Database setup
- [ ] Backend API (Module 1)
- [ ] Backend API (Module 2)
- [ ] Frontend - Home/Dashboard
- [ ] Frontend - Core Feature 1
- [ ] Frontend - Core Feature 2
- [ ] Authentication module
- [ ] Integration of frontend & backend

**Phase 3 Completion:** 0 / 8 tasks ⬜

---

### Phase 4: Testing
- [ ] Unit tests written & passing
- [ ] Integration tests
- [ ] User acceptance testing (UAT)
- [ ] Bug fixes & patches
- [ ] Performance testing

**Phase 4 Completion:** 0 / 5 tasks ⬜

---

### Phase 5: Submission
- [ ] Final code pushed to GitHub
- [ ] Project report written
- [ ] Presentation slides ready
- [ ] Demo video (if required)
- [ ] Submission to department

**Phase 5 Completion:** 0 / 5 tasks ⬜

---

##  Weekly Milestone Checkboxes

| Week | Dates | Planned Goal | Done? |
|------|-------|-------------|-------|
| Week 1 | | Requirement gathering | ⬜ |
| Week 2 | | System design start | ⬜ |
| Week 3 | | Design complete | ⬜ |
| Week 4 | | DB setup + Backend start | ⬜ |
| Week 5 | | Backend 50% | ⬜ |
| Week 6 | | Backend complete | ⬜ |
| Week 7 | | Frontend start | ⬜ |
| Week 8 | | Frontend 50% | ⬜ |
| Week 9 | | Frontend complete | ⬜ |
| Week 10 | | Integration | ⬜ |
| Week 11 | | Testing | ⬜ |
| Week 12 | | Bug fixes + Documentation | ⬜ |
| Week 13 | | Final submission | ⬜ |

---

##  Sprint Planning

### Sprint 1 (Week 1–2)
| Task | Assignee | Story Points | Status |
|------|----------|-------------|--------|
| | | | ⬜ |
| | | | ⬜ |
| | | | ⬜ |

### Sprint 2 (Week 3–4)
| Task | Assignee | Story Points | Status |
|------|----------|-------------|--------|
| | | | ⬜ |
| | | | ⬜ |
| | | | ⬜ |

### Sprint 3 (Week 5–6)
| Task | Assignee | Story Points | Status |
|------|----------|-------------|--------|
| | | | ⬜ |
| | | | ⬜ |
| | | | ⬜ |

### Sprint 4 (Week 7–8)
| Task | Assignee | Story Points | Status |
|------|----------|-------------|--------|
| | | | ⬜ |
| | | | ⬜ |
| | | | ⬜ |

---

##  Issues / Blockers Log

| Date | Issue Description | Severity | Assigned To | Resolved? |
|------|------------------|----------|-------------|-----------|
| | |  High | | ⬜ |
| | |  Medium | | ⬜ |
| | |  Low | | ⬜ |

> [!warning] Blocker Protocol
> If a blocker is **High severity** and unresolved for > 2 days, escalate to the mentor immediately.

---

##  Overall Progress

```
Planning:    [          ] 0%
Design:      [          ] 0%
Development: [          ] 0%
Testing:     [          ] 0%
Submission:  [          ] 0%

TOTAL:       [          ] 0%
```

---

## ️ Meeting Notes

### Mentor Meeting 1
- **Date:**
- **Attendees:**
- **Key Feedback:**
- **Action Items:**

### Mentor Meeting 2
- **Date:**
- **Attendees:**
- **Key Feedback:**
- **Action Items:**

---

*Last updated: 2026-06-16 | Semester V TY B.Sc. CS*
