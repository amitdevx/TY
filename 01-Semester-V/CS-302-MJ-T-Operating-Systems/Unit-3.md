...
graph TD
    ...
    MMU -->|"if 346 >= 400"| TRAP["TRAP: Segmentation<br/>Fault!"]
```
...
```mermaid
graph TD
    subgraph LA["Logical Address Space<br/>(Process)"]
        P0["Page 0<br/>(0-3)"]
        P1["Page 1<br/>(4-7)"]
        P2["Page 2<br/>(8-11)"]
        P3["Page 3<br/>(12-15)"]
...
```mermaid
graph TD
    subgraph LA["Logical Address<br/>(s, d)"]
        S["Segment # (s)"]
        D["Offset (d)"]
        S ~~~ D
    end
    ...
    ST --> |"if d >= Limit[s]"| TRAP["TRAP: Segmentation<br/>Fault"]
```
...
```mermaid
flowchart TD
    A["Process references<br/>page"] --> B{"Valid bit<br/>in page table?"}
    B -->|"Valid (in<br/>memory)"| G["Access granted<br/>"]
    B -->|"Invalid (on<br/>disk)"| C["PAGE FAULT TRAP<br/>(interrupt to OS)"]
    C --> D["Check if valid reference<br/>or out-of-bounds?"]
    D -->|"Out of bounds"| E["Terminate process<br/>(Segmentation Fault)"]
    D -->|"Valid reference"| F["Find free frame in<br/>memory"]
