---
title: "Weekly Review - Week {{week_number}}"
date: {{date}}
week_number: {{week_number}}
week_start: {{week_start_date}}
week_end: {{week_end_date}}
semester: "{{semester}}"
academic_year: "2026–27"
overall_mood: "{{mood}}"
# mood options:  Great |  Good |  Okay |  Rough
productivity_score: "{{score}}/10"
tags:
  - weekly-review
  - tracking
  - "{{semester}}"
  - week-{{week_number}}
aliases:
  - "Week {{week_number}} Review"
---

#  Weekly Review - Week {{week_number}}

> [!note] ️ Week Overview
> **Week:** {{week_start_date}} → {{week_end_date}}
> **Semester:** {{semester}} | **Week #:** {{week_number}}
> **Mood:** {{mood}} | **Productivity:** {{score}}/10

---

##  Weekly Goals Review

> How did you do on last week's goals?

| Goal | Set On | Status | Notes |
|---|---|---|---|
|  | [[00-Dashboard/Home]] |  Done /  Not Done |  |
|  |  |  Done /  Not Done |  |
|  |  |  Done /  Not Done |  |
|  |  |  Done /  Not Done |  |
|  |  |  Done /  Not Done |  |
|  |  |  Done /  Not Done |  |

**Goals Completion Rate:** ___/___  (___%)

---

##  Subjects Studied This Week

| Subject | Hours Spent | Topics Covered | Units Completed | Notes |
|---|---|---|---|---|
| CS-301 Theory of Computation | hrs | | | |
| CS-302 Advanced Java | hrs | | | |
| CS-303 Microprocessors | hrs | | | |
| CS-304 Software Engineering | hrs | | | |
| CS-305 Computer Networks | hrs | | | |
| CS-306 Elective I | hrs | | | |
| Mathematics | hrs | | | |
| **Total** | **hrs** | - | - | - |

---

##  Topics Completed This Week

###  Theory Topics

- [ ] 
- [ ] 
- [ ] 
- [ ] 
- [ ] 

###  Lab Work

- [ ] 
- [ ] 
- [ ] 

###  Assignments / Projects

- [ ] 
- [ ] 

---

##  Pending Topics (Carried Forward)

> [!warning]  These need to be addressed next week!

| Topic | Subject | Priority | Reason Pending |
|---|---|---|---|
|  |  |  High /  Med /  Low |  |
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |

---

##  Lab Progress This Week

| Lab | Experiment # | Title | Status | Submitted? |
|---|---|---|---|---|
|  |  |  |  /  /  |  |
|  |  |  |  |  |
|  |  |  |  |  |

**Lab Journal Status:**
- CS-305 Java Lab: ___/8 submitted
- CS-306 SE Lab: ___/6 submitted
- CS-308 Networks Lab: ___/6 submitted

---

##  Challenges Faced

> [!warning] Issues Encountered This Week

### Academic Challenges

1. **Challenge:** 
   **How I Dealt With It:** 

2. **Challenge:** 
   **How I Dealt With It:** 

3. **Challenge:** 
   **How I Dealt With It:** 

### Personal / Time Management Challenges

1. 
2. 

---

##  What Went Well

> [!tip]  Wins of the Week

-  
-  
-  
-  

---

##  Plan for Next Week

> Goals and tasks for Week {{week_number + 1}}

###  Academic Goals

- [ ] 
- [ ] 
- [ ] 
- [ ] 
- [ ] 

###  Lab Goals

- [ ] 
- [ ] 

###  Assignment / Project Goals

- [ ] 
- [ ] 

###  Priority Topics

| Priority | Subject | Topic | Target Date |
|---|---|---|---|
|  High |  |  |  |
|  Medium |  |  |  |
|  Low |  |  |  |

---

##  Resources Used This Week

| Resource | Type | Subject | Usefulness (1–5) |
|---|---|---|---|
|  | Book / Video / Website / Notes |  |  |
|  |  |  |  |
|  |  |  |  |
|  |  |  |  |

---

##  Reflection

> [!note]  Weekly Reflection
>
> **Best moment of the week:**
>
> **Biggest lesson learned:**
>
> **One thing I'd do differently:**
>
> **Motivation level going into next week (1–10):** /10

---

##  Navigation

| | Link |
|---|---|
| ⬅️ Previous Week | [[Week {{week_number - 1}} Review]] |
| ️ Next Week | [[Week {{week_number + 1}} Review]] |
|  Grade Tracker | Grade-Tracker |
|  Habit Tracker | Habit-Tracker |
|  Dashboard | [[00-Dashboard/Home]] |

---

*Review written on: {{date}} | Week {{week_number}} of Semester {{semester}}*
